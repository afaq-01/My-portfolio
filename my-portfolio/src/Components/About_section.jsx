...
const info = [
  ...
  { icon: Mail, label: "Email:", value: "afaqahmad.dev@gmail.com" },
];
...
function SkillCard({ name, icon: Icon, color, delay }) {
  ...
  return (
    <div
      ...
      className={`flex flex-col items-center justify-center gap-3 py-6 rounded-xl border border-neutral-800 bg-neutral-900/60 hover:border-purple-500/50 transition-all duration-500 ${isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
        }`}
    >
      <Icon size={32} color={color} />
      <span className="text-xs text-neutral-300 text-center px-2">{name}</span>
    </div>
  );
}
  ...
  return (
    <section className="bg-black py-20 overflow-hidden">
      <div className="max-w-6xl mx-auto px-6 md:px-10 grid md:grid-cols-2 gap-16">
        ...
        >
          <span className="text-xs font-medium text-purple-500 tracking-wide mb-3">
            ABOUT ME
          </span>
          ...
          <a
            href="/resume.pdf"
            download
            className="flex items-center gap-2 text-sm font-medium border border-purple-700 text-purple-400 px-6 py-3 rounded-md hover:bg-purple-600 hover:text-white hover:border-purple-600 transition-colors"
          >
            Download Resume
            <Download size={16} />
          </a>
